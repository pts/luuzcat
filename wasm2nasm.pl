...
while (<STDIN>) {
  ...
  if (s@^\t(?!\t)@@) {  # Assembly instructions in _TEXT.
    ...
    s@\b(byte|d?word) ptr (\w[^,]*)@$1 [$2]@g;
    s@\b(byte|d?word) ptr (\[[^\\[\]]*?\])@$1 $2@g;
    s@\boffset @@g;
    if (m@^(CALL|JMP) near ptr (L\@\d+\@\d+|\w+)$@) { $_ = "$1 $2" }
    #else { die("bad instruction: $_\n") }
    print("\t\t", $_, "\n");
  } elsif (m@^\t\t@) {
    if (m@^\t\tPUBLIC\s+(\w+)$@) { print("f_global $1\n") }
    elsif (m@^\t\tEXTRN\s+(\w+):@) { print("%define __NEED_$1\n") }
    elsif (m@^\t\tASSUME\s+@) {}
    elsif ($_ eq "\t\tEND") {}
    else { die("bad: $_\n") }
  } else {
    if (m@^(\w+)(?::|\s+LABEL\s)@) { print("$1:\n") }
    elsif (m@^\@\$(\d+)(?::|\s+LABEL\s)@) { print("L\@$fi\@$1:\n") }
    elsif (m@^    D[BWD]\s+@) { s@^\s+@\t\t@; die("bad quote: $_\n") if m@["\x27]@; s@\@\$(\d+)\b@L\x40$fi\x40$1@g if s@\boffset (?:FLAT:)?@@g; print($_, "\n") }
